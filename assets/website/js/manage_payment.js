...
function payNow(upi_id,rs) {
    ...
    var upi_address = "fcbizbpwual@freecharge";
    var amt = parseFloat(rs).toFixed(2);
    var trid=Math.floor((Math.random() * 1000) + 100)+"_"+Date.now();
    switch (payType) {
        case 'gpay':
            redirect_url = "tez://upi/pay?pa=" + upi_address + "&pn=" + site_name + "&am=" + amt + "&tr="+ trid +"&mc=8931&cu=INR&tn=" + site_name;
            break;

        case 'phonepe':
            redirect_url = "phonepe://pay?pa=" + upi_address + "&pn=" + site_name + "&am=" + amt + "&tr="+ trid +"&mc=8931&cu=INR&tn=" + site_name;
            break;

        case 'paytm':
            redirect_url = "paytmmp://pay?pa=" + upi_address + "&pn=" + site_name + "&am=" + amt + "&tr="+ trid +"&mc=8931&cu=INR&tn=" + site_name;
            break;

        case 'bhim_upi':
            redirect_url = "bhim://pay?pa=" + upi_address + "&pn=" + site_name + "&am=" + amt + "&tr="+ trid +"&mc=8931&cu=INR&tn=" + site_name;
            break;

        case 'whatspp_pay':
            redirect_url = "whatsapp://pay?pa=" + upi_address + "&pn=" + site_name + "&am=" + amt + "&tr="+ trid +"&mc=8931&cu=INR&tn=" + site_name;
            break;
